...
function getStageMessage(state) {
  switch (state.stage) {
    ...
    case 'repay':
      return 'Your previous online checkout was not completed. You can reopen payment and pay again.'
    case 'under_review':
...
function Payments() {
  ...
  const [receiptInputKeys, setReceiptInputKeys] = useState({})
  const [workingReceiptId, setWorkingReceiptId] = useState('')
  ...
  const openCheckout = async (response) => {
    ...
    const razorpay = new window.Razorpay({
      ...
      modal: {
        ondismiss: async () => {
          await loadData()
          setStatus({ type: 'error', message: 'Payment was cancelled. Click Repay to reopen checkout.' })
        },
      },
    })
  ...
  const openReceipt = async (payment) => {
    try {
      await openFileFromApi(`/api/payments/${payment._id}/receipt`)
    } catch (error) {
      setStatus({ type: 'error', message: extractApiError(error) })
    }
  }

  const updateReceiptDraft = (paymentId, patch) => {
  ...
  const uploadReceipt = async (event, payment) => {
    event.preventDefault()
    const draft = receiptDrafts[payment._id] || {}

    if (!draft.file) {
      setStatus({ type: 'error', message: 'Please choose the payment receipt before uploading.' })
      return
    }

    setWorkingReceiptId(payment._id)
    setStatus({ type: '', message: '' })

    try {
      const payload = new FormData()
      payload.append('transactionId', draft.transactionId || payment.transactionId || '')
      payload.append('receipt', draft.file)

      const { data } = await api.post(`/api/payments/${payment._id}/receipt-upload`, payload, {
        headers: {
      ...
      setReceiptDrafts((current) => ({
        ...current,
        [payment._id]: { transactionId: '', file: null },
      }))
      setReceiptInputKeys((current) => ({
        ...current,
        [payment._id]: (current[payment._id] || 0) + 1,
      }))
      setStatus({ type: 'success', message: data.message || 'Receipt uploaded successfully.' })
    } catch (error) {
      setStatus({ type: 'error', message: extractApiError(error) })
    } finally {
      setWorkingReceiptId('')
              ...
              <button className="button button-primary" disabled={submitting || !selectedService} type="submit">
                {submitting
                  ? 'Processing...'
                    ...
                    : selectedServiceState?.eligibility.hasRepayableOnlinePayment
                      ? 'Repay'
                      : 'Pay Now'}
              </button>
            </form>
          ...
          <div className="list-stack">
            {payments.map((payment) => (
              <div className="card-inline" key={payment._id}>
                ...
                {payment.status !== 'paid' && payment.verificationStatus !== 'verified' ? (
                  <form className="payment-receipt-upload" onSubmit={(event) => uploadReceipt(event, payment)}>
                    <label>
                      Transaction ID / UPI reference
                      <input
                        onChange={(event) => updateReceiptDraft(payment._id, { transactionId: event.target.value })}
                        placeholder="Enter transaction reference"
                        required
                        type="text"
                        value={receiptDrafts[payment._id]?.transactionId ?? payment.transactionId ?? ''}
                      />
                    </label>
                    <label>
                      ...
                      <input
                        ...
                        key={receiptInputKeys[payment._id] || 0}
                        onChange={(event) => updateReceiptDraft(payment._id, { file: event.target.files?.[0] || null })}
                        required
                        type="file"
                      />
                    </label>
                    <button
                      ...
                      disabled={workingReceiptId === payment._id}
                      type="submit"
                    >
                      {workingReceiptId === payment._id ? 'Uploading...' : 'Upload Receipt for Verification'}
                    </button>
                  </form>
                ) : null}
                {payment.status === 'paid' || payment.verificationStatus === 'verified' ? (
                  <button className="button button-secondary button-compact" onClick={() => openReceipt(payment)} type="button">
                    Receipt
                  </button>
                ) : null}
              </div>
