...
function AdminClientWorkspace({ section = 'overview' }) {
  ...
  const approveDocument = async (documentRecord) => {
    setWorkingKey(`document-${documentRecord._id}`)

    try {
      await api.patch(`/api/admin/documents/${documentRecord._id}`, {
        status: 'approved',
        remarks: documentRecord.remarks || 'Approved by admin.',
      })
      await loadUser()
  ...
  const sendPreparedFile = async (payment) => {
    ...
    try {
      ...
      payload.append('documentType', draft.title || `Prepared file - ${payment.serviceType}`)
      ...
      setStatus({ type: 'success', message: 'Prepared file sent to the client.' })
    } catch (error) {
      setStatus({ type: 'error', message: extractApiError(error) })
    } finally {
      setWorkingKey('')
                ...
                return (
                  <article className="admin-folder-file-row admin-folder-file-row-rich" key={documentRecord._id}>
                    <div>
                      ...
                      <p>
                        {documentRecord.inputType === 'text'
                          ? documentRecord.textValue || 'Text details submitted'
                    ...
                    <div className="admin-record-actions admin-service-document-actions">
                      {documentRecord.status !== 'approved' ? (
                        <button
                          ...
                          onClick={() => approveDocument(documentRecord)}
                          type="button"
                        >
                          {workingKey === `document-${documentRecord._id}` ? 'Approving...' : 'Approve'}
                        </button>
                      ) : null}
                      {documentRecord.inputType === 'text' ? null : (
                        <>
                        <button
                        ...
                        <a
                          className="button button-ghost button-compact admin-document-download"
        ...
        {user.payments.map((payment) => {
          ...
          return (
            <article className="admin-record-card" key={payment._id}>
              ...
              {payment.verificationStatus === 'verified' || payment.status === 'paid' ? (
                <div className="admin-inline-editor admin-inline-editor-wrap">
                  <label>
                    Prepared file title
                    <input
                      onChange={(event) => updatePreparedFileDraft(payment._id, { title: event.target.value })}
                      placeholder={`Prepared file - ${payment.serviceType}`}
                      type="text"
                      value={preparedFileDrafts[payment._id]?.title || ''}
                    />
                  </label>
                  <label>
                    Prepared file
                    <input
                      accept=".pdf,image/*"
                      key={preparedFileInputKeys[payment._id] || 0}
                      onChange={(event) => updatePreparedFileDraft(payment._id, { file: event.target.files?.[0] || null })}
                      type="file"
                    />
                  </label>
                  <label className="grow">
                    Note to client
                    <input
                      onChange={(event) => updatePreparedFileDraft(payment._id, { notes: event.target.value })}
                      placeholder="Prepared work file shared by admin."
                      type="text"
                      value={preparedFileDrafts[payment._id]?.notes || ''}
                    />
                  </label>
                  <button
                    className="button button-primary button-compact"
                    disabled={workingKey === `prepared-${payment._id}`}
                    onClick={() => sendPreparedFile(payment)}
                    type="button"
                  >
                    {workingKey === `prepared-${payment._id}` ? 'Sending...' : 'Send Prepared File'}
                  </button>
                </div>
              ) : null}
            </article>
