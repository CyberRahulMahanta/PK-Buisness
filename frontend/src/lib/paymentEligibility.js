...
export function getServicePaymentEligibility({ service, documents = [], payments = [] }) {
  ...
  const hasApprovedPaymentDocument = approvedDocuments > 0 && rejectedDocuments === 0
  const hasPendingPayment = servicePayments.some(
    (payment) =>
      (payment.paymentMethod !== 'online' || payment.razorpayPaymentId) &&
      (payment.verificationStatus === 'pending' || payment.status === 'pending'),
  )
  const hasRepayableOnlinePayment = servicePayments.some(
    (payment) =>
  ...
  if (isServiceCompleted) {
    ...
    stage = 'approved'
  } else if (hasRepayableOnlinePayment) {
    stage = 'repay'
