...
$completionDate = $value($body, ['completionDate', 'completion_date', 'date']);
$visitLink = $value($body, ['visitLink', 'visit_link', 'websiteUrl', 'website_url'], 'https://www.pkbusinesssolution.in');
...
if ($customerName === '' || $serviceName === '' || $completionDate === '') {
    http_response_code(422);
    echo json_encode([
        'error' => 'Customer name, service name, and completion date are required.',
    ]);
    exit;
}

// Build WhatsApp service completion template payload
$payload = json_encode([
    'phone'       => $phone,
    'auth_token'  => 'de955238e3dba02bd8137e535adc70de',
    'template_id' => '52bd4930-2876-4bbc-bd23-e9e6b99568be',
    'header_vars' => [],
    'body_vars'   => [
        1 => $customerName,
        2 => $serviceName,
...
http_response_code(200);
echo json_encode(['message' => 'Service WhatsApp message sent successfully.']);
